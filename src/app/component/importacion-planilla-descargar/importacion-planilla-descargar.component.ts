...
@Component({
  ...
  providers: [confGeneralService, ExportarService]
})
export class ImportacionPlanillaDescargarComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit() {
    this.idversion = this.data.idversion;
    this.gettipolinea();
    this.getzona();
  }
  ...
  guardar(newForm) {
    ...
    let nomT = "";
    let nomZ = "";
    this.tipolinea.forEach(element => {
      if( element.n_idpl_tipolinea == this.idtipolinea ){
        nomT = element.c_nombre
      }
    });
    this.zona.forEach(element => {
      if( element.n_idpl_zona == this.idzona ){
        nomZ = element.c_nombre
      }
    });
    ...
    this._exportar_service.exportar(request, this.getToken().token).subscribe(
      result => {    
        ...
        workbook.xlsx.writeBuffer().then((data: any) => {
          ...
          fs.saveAs(blob, 'Datos-'+nomT +'-'+ nomZ + '.xlsx');
